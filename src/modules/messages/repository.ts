...
type MessagesSelect = Selectable<Messages>
interface MessageRepository {
  findByUsername(username: string): Promise<MessagesSelect[]>;
  findBySprint(sprint: string): Promise<MessagesSelect[]>;
  findBySprintAndUsername(sprint: string, username: string): Promise<MessagesSelect[]>;
  findAll(): Promise<MessagesSelect[]>;
}
export default (db: Database) : MessageRepository => ({
  findAll: async ():Promise<MessagesSelect[]> => db.selectFrom('messages').selectAll().execute(),
  ...
  findBySprint: async (ids: string) =>
    db.selectFrom('messages').selectAll('messages').where('sprintId', '=', ids).execute(),
